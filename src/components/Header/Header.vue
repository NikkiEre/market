  ...
  <header class="container__page header" v-if="!isLoginPage">
      <div class="header__container">
        <div class="header__logo">
            ...
            <nav>
                <button class="header__button header__link" @click="showMenu">
                    <svg 
                ...
                <a @click.prevent="moveToLoginOrReg" :class="['header__link', 'header__link_auth', isAuth ? 'header__link_ava' : '']">
                    <svg v-if="!isAuth"
                ...
                <router-link to="/cart" class="header__link header__link_cart">
                    <svg 
                        width="32" height="29" viewBox="0 0 32 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="header__count" v-if="countItemsInCart">
                        {{countItemsInCart}}
                    </span>
                </router-link>
            </nav>
            ...
            <Menu 
                ...
                :moveToLoginOrReg="moveToLoginOrReg"
            />
        </div>
  </header>
</template>
...
export default {
  components: { Menu },
    ...
    computed: {
        countItemsInCart() {
            return store.getters.GET_CART.length;
        },
        isLoginPage() {
            return this.$route.name == "login";
        },
        isAuth() {
            return !!store.getters.GET_USERDATA.email;
        },
        avaUser() {
            const userInfo = store.getters.GET_USERDATA;
            return userInfo.first_name[0].toLocaleUpperCase() + userInfo.last_name[0].toLocaleUpperCase();
        }
    },
    methods: {
